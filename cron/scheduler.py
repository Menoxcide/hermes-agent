...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # Smart Provider Resilience: Try fallbacks if the primary provider fails
        runtime = None
        requested_provider = job.get("provider") or os.getenv("HERMES_INFERENCE_PROVIDER")
        ...
        try:
            ...
            logger.warning("Job '%s': Primary provider '%s' failed. Attempting fallbacks...", job_id, requested_provider)
            
            # Try Fallback List from config
            fallbacks = _cfg.get("fallback_providers", [])
            if isinstance(fallbacks, str): fallbacks = [fallbacks]
            
            for fb in fallbacks:
                try:
                    logger.info("Job '%s': Trying fallback provider: %s", job_id, fb)
                    runtime = resolve_runtime_provider(requested=fb)
                    if runtime: break
                except:
                    continue
            
            if not runtime and requested_provider != "JUSTIN":
                try:
                    logger.info("Job '%s': All fallbacks failed. Forcing local JUSTIN node.", job_id)
                    runtime = resolve_runtime_provider(requested="JUSTIN")
                except Exception as final_exc:
                    message = format_runtime_provider_error(exc)
                    logger.error("Job '%s': Critical failure. No functional providers found.", job_id)
                    raise RuntimeError(message) from final_exc
            elif not runtime:
                message = format_runtime_provider_error(exc)
